#ifndef SSM_RESTRICTED_VORONOI_DIAGRAM_TRIANGLE_MESH_METRIC_TRAITS_H
#define SSM_RESTRICTED_VORONOI_DIAGRAM_TRIANGLE_MESH_METRIC_TRAITS_H
...
class Triangle_mesh_metric_traits {
   public:
    using Kernel = K;
    using Graph = G;
    ...
    struct Metric_any_intersected_face {
        std::optional<face_descriptor> operator()(const Data& data, const Vector_3& d) const {
            ...
            Ray_3 ray(ORIGIN, d);
            for (const auto& [fd, t] : data) {
                if (do_intersect(ray, t)) return fd;
            }
            return std::nullopt;
        }
    };
...
#endif  // SSM_RESTRICTED_VORONOI_DIAGRAM_TRIANGLE_MESH_METRIC_TRAITS_H
